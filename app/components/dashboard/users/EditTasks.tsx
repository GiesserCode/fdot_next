const EditTasks = ({item}: any) => {
    return <>{item.tasks?.map((thing: any, number: number) => (
        <form key={number} className={`bg-normalBG border border-lightBG p-2 rounded-xl mb-4`}>
            ...
            <div className={`flex`}>uuid:
                <input type="text" className={`no-focus normal-input`} disabled={true} defaultValue={thing.uuid}
                       name={"uuid"}/>
            </div>
            <div className={`flex`}>id:
                <input type="text" className={`no-focus normal-input`} disabled={true} defaultValue={item.id}
                       name={"id"}/>
            </div>
