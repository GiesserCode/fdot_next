...
import {EyeSVG, HinzufügenSVG} from "@/app/ui/SVG";
...
function Nachrichten({contacts}: any) {
    ...
    return contacts[0] && (
        <>
        <Link className={`w-full border border-lightBG rounded-xl p-4 my-2 text-xl mb-10 flex justify-between text-primary bg-normalBG cursor-pointer`} href={'/dashboard/users'}>
        ...
        <div className={`w-full overflow-hidden`}>
            ...
            <div className={`bg-green-600 rounded-xl p-2 absolute top-10 right-5 transition duration-500 ease-in-out ${sent ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"} flex gap-2`}>
                <svg
                    ...
                    <div key={index}
                        ...
                        <div className={`lg:flex w-full`} onClick={() => toggleTextarea(index)}>
                            ...
                            <div className={`lg:w-1/3 text-end flex justify-end gap-2 items-center`}>
                                {item.time}
                                <div className={`${visibleTextarea[index] ? "visible" : "hidden"} ${ item.read ? "bg-red-600" : "bg-green-600"} bg-opacity-60 rounded-lg z-20`} onClick={() => setRead(item.id, item.read)}>
                                    <EyeSVG/>
                                </div>
                            </div>
                        </div>
                        <div className={`w-full flex ${visibleTextarea[index] || "hidden"} max-lg:flex-col`}>
                        <textarea className={`text-2xl mt-5 no-focus w-1/2 bg-transparent max-lg:w-full`} rows={7} readOnly={true}
                                  defaultValue={item.message}></textarea>
                            <form className={`w-1/2 max-lg:w-full`} action={(e) => sendNotes(e, item.id, item.notes)} onSubmit={handleSubmit}>
