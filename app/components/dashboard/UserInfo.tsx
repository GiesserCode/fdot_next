...
const Clock = dynamic(() => import("@/app/components/dashboard/Clock"), {
  ssr: false,
});
const UserInfo = async ({ users }: any) => {
  return (
    <div className={`w-full grid place-items-center overflow-x-hidden maxW`}>
      <div
        className={`w-full flex justify-between max-lg:flex-col max-lg:gap-5 mb-5`}
      >
        <div
          className={`w-[30%] grid place-items-center bg-normalBG rounded-xl p-5 max-lg:w-full`}
        >
...
function Notes({ users }: any) {
  return (
    users.notes && (
      <div className={`w-full text-start my-10 text-2xl text-secondary`}>
        <textarea
          className={`text-start no-focus normal-input w-full max-lg:h-64`}
          rows={6}
          disabled={true}
        >
          {users.notes}
